...
def eval_result(dir,logger):
    ...
    logger.info('属性抽取: {} recall: {} f1: {}'.format(p, r, f1))
    with open(output_file, 'w', encoding='utf-8') as fp:
        fp.write('P: ' + str(p) + ' R: ' + str(r) + ' f1: ' + str(f1) + '\n')
        # fp.write(' '.join(output_lines))
    return f1

def eval_ts_result(dir,logger):
    input_file = os.path.join(dir, 'pre.txt')
    ...
    logger.info('属性抽取: {} recall: {} f1: {}'.format(ad_p, ad_r, ad_f1))
    logger.info('联合任务： precision: {} recall: {} f1: {}'.format(ads_p, ads_r, ads_f1))
